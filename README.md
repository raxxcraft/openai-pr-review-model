...
You can run the action locally using the `act` tool which emulates GitHub Actions.

1. Install act: see https://github.com/nektos/act

2. Create a `.secrets` file in the project root with the secrets you want to provide. Example:

```text
GITHUB_TOKEN=ghp_xxxyourtokenxxx
OPENAI_API_KEY=sk-xxxyouropenaiapikeyxxx
```

3. Build and package, then run the `act` helper script (Windows PowerShell example):
...
- If tests or the code try to access GitHub Action-only environment variables (for example the Octokit `auth-action` helper), you'll see errors like `GITHUB_ACTION environment variable is not set` when running `npm test` locally. Use the `test-standalone.js` script to exercise core classes without the GitHub Actions environment:

```powershell
node test-standalone.js
```

- The TypeScript build may fail if compiled JS files would overwrite existing source JS files in the repository (this repo contains pre-built `lib/*.js` alongside `src/*.ts`). If you see errors such as `Cannot write file ... because it would overwrite input file`, either remove or move the conflicting files, or adjust `tsconfig.json` `outDir`/`rootDir` so `lib` is generated into a separate folder.
